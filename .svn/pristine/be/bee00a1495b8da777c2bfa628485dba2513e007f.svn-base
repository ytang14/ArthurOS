...
/* TEST 4 err_open
 * tries to open three files with wrong file names
 * prints "[TEST_NAME]: PASS" if behavior is EXPECTED
 *     and then returns 0
 * prints "[TEST_NAME]: FAIL" if behavior is UNEXPECTED
 *     and then returns 2
...
int err_vidmap(void) {
	...
	if (-1 != ece391_vidmap((unsigned char**)0x0)) {
		ece391_fdputs (1, (uint8_t*)"Null pointer fail\n");
        fail = 2;
	}
	
	if (-1 != ece391_vidmap((unsigned char**)0x400000)) {
